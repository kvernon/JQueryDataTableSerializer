...
{
    [TestFixture]
    public class AndDataPropertiesPopulated
    {
        ...
        [Test]
        public void DataShouldBeMatchingInterface()
        {
            var expected = typeof(IDataTableSerializer);

            Assert.AreEqual(expected, _wrapper.Data[0].GetType().GetInterfaces().Single(x => x.Name.Contains(expected.Name)));
        }
    }
}